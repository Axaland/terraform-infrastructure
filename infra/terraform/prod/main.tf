...
data "aws_ecr_repository" "service" {
  name = var.service_ecr_repository
}

locals {
  service_image = length(trimspace(var.service_image)) > 0 ? var.service_image : "${data.aws_ecr_repository.service.repository_url}:${var.service_image_tag}"
}
...
resource "aws_security_group" "vpc_endpoints" {
  name        = "${var.env_name}-vpce-sg"
  description = "Accesso ai VPC endpoint interni"
...
module "security_baseline" {
  ...
  env                            = var.env_name
  config_snapshot_retention_days = 180
