...
module "rds" { source = "../modules/rds_postgres" env = var.env_name db_name = var.db_name username = var.db_user subnet_ids = module.vpc.private_subnet_ids vpc_id = module.vpc.vpc_id backup_retention = 7 allowed_cidr_blocks = var.rds_allowed_cidrs }

module "ecr" { source = "../modules/ecr" repositories = ["app"] }

module "service" { source = "../modules/ecs_fargate_service" env = var.env_name service_name = "app" image = var.service_image private_subnet_ids = module.vpc.private_subnet_ids vpc_id = module.vpc.vpc_id secrets = { DB_PASSWORD = module.rds.db_secret_arn } desired_count = 2 min_capacity = 2 max_capacity = 6 }
...
module "security_baseline" { source = "../modules/security_baseline" env = var.env_name config_snapshot_retention_days = 120 enable_guardduty = true enable_config = true required_tags = ["Environment","Owner","CostCenter"] }
